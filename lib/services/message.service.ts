...
export async function getManagementListChat(): Promise<
  ManagementMessageBoxDTO[]
> {
  try {
    const response = await fetch(`${BASE_URL}/message/getManagementBoxChat`);
    ...
    // Mapping the response to ItemChat
    const chat: ManagementMessageBoxDTO[] = rawData.box
      .map((box: any) => {
        // Filter out the current logged-in user from receiverIds

        const reveiver = box.receiverIds.map((receiver: any) => ({
          id: receiver._id,
        ...
        return {
          _id: box._id,
          receiverIds: reveiver,
          createAt: box.createAt,
        };
      })
      .filter((item): item is ManagementMessageBoxDTO => item !== null); // Filter out null values and ensure the type is ItemChat
...
export async function getAllChat(boxId: string): Promise<ChatResponse[]> {
  try {
    const response = await fetch(
      `${BASE_URL}/message/getAllChatManagement?boxId=${boxId}`
    );
